...
class _MockTestPageState extends ConsumerState<MockTestPage> {
  ...
  final Map<String, int?> _answers = {}; // null = boş
  ...
  Widget build(BuildContext context) {
    ...
    final currentTest = testState.currentTest;

    if (testState.isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Deneme Testi'),
        ),
        body: const Center(child: CircularProgressIndicator()),
      );
    }

    if (testState.error != null || currentTest == null) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Deneme Testi'),
        ),
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                testState.error ?? 'Test bulunamadı',
  ...
  void _finishTest(BuildContext context) {
    final testState = ref.read(mockTestProvider);
    final test = testState.currentTest!;

    // Test sonuçlarını hesapla
    ref.read(mockTestProvider.notifier).finishTest(_answers);

    // Sonuç sayfasına yönlendir
    context.pushReplacement('/student/deneme-kitaplari/result');
  }
}
